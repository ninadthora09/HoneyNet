...
export default function Credentials() {
  ...
  const totalAttempts = credentials.reduce((acc, curr) => acc + curr.count, 0);

  const totalIPs = new Set(credentials.flatMap((c) => c.sourceIPs || [])).size;
          ...
          <div>
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <h1 className="text-4xl font-black">Credential Intelligence</h1>
              </div>
            </div>

            <p className="text-slate-400 text-lg max-w-2xl">
              Aggregated attacker credential combinations captured from live SSH
              and HTTP honeypot activity.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-[#080f1a] border border-slate-800 rounded-2xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-slate-400">Credential Pairs</h3>
          </div>
        ...
        <div className="bg-[#080f1a] border border-slate-800 rounded-2xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-slate-400">Total Attempts</h3>
          </div>

          <p className="text-4xl font-black">{totalAttempts}</p>
        </div>

        <div className="bg-[#080f1a] border border-slate-800 rounded-2xl p-5">
          <div className="flex items-center gap-3 mb-4">
            <Globe className="text-cyan-400" size={22} />
            <h3 className="text-slate-400">Unique Attackers</h3>
          </div>

          <p className="text-4xl font-black">{totalIPs}</p>
        </div>
      </div>
      ...
      <div className="bg-[#080f1a] border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800 bg-slate-900/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold">Captured Credentials</h2>
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      <div className="inline-flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 rounded-xl px-4 py-2 font-mono">
                        <span>{cred.username || "unknown"}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-5 text-slate-400 text-sm whitespace-nowrap">
                      {new Date(cred.firstSeen).toLocaleString()}
                    </td>

                    {/* LAST SEEN */}

                    <td className="px-6 py-5 text-slate-400 text-sm whitespace-nowrap">
                      {new Date(cred.lastSeen).toLocaleString()}
                    </td>
                  </tr>
                ))
              )}
